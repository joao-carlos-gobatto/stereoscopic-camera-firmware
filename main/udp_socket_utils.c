...
void udp_discovery_listener_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "Aguardando broadcasts de discovery na porta %d...", DISCOVERY_PORT);
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            if (strcmp(rx_buf, BROADCAST_MSG) == 0) {
                ESP_LOGI(TAG, "Discovery recebido de %s", inet_ntoa(sender_addr.sin_addr));
